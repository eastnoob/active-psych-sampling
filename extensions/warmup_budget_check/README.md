...
在 Phase1 Step3 的实现中，我们新增了“智能因子列选择”逻辑：

- 优先识别并使用匹配 `x1`, `x2`, ..., `xN` 的列（正则表达式 ^x\\d+）。
- 如果未找到任何 `xN` 格式的列，则退回到使用所有非 `subject`/`response` 列。
- 若存在排除的列（不符合 `xN`）将打印 `信息` 日志，告知被排除的列名。

目的：大多数设计空间使用 `xN` 命名惯例，这个改动使 Phase1 Step3 更方便地自动识别自变量，减少手动配置错误。

影响：如果你依赖 Phase1 Step3 处理所有非 `xN` 列（而不是仅 xN），请注意此行为差异；你可以通过重命名列或显式传入设计空间来避免影响。
