...
class Phase1DataAnalyzer:
    ...
    def __init__(
        ...
        if data_path.is_dir():
            ...
            logger.info(f"Reading subject data from directory: {data_csv_path}")
            ...
            # Read each subject file and add subject column
            all_dfs = []
            for i, csv_path in enumerate(subject_csvs, start=1):
                ...
                if subject_col not in df_subject.columns:
                    # Extract subject ID from filename (subject_1.csv -> subject_1)
            ...
            logger.info(f"  Total after merging: {len(self.df)} rows")

        else:
            # File mode: direct read
            logger.info(f"Reading data file: {data_csv_path}")
        ...
        logger.info(f"  Subjects: {len(np.unique(self.subject_ids))}")
        logger.info(f"  Factors: {len(self.factor_cols)}")
        logger.info(f"  Factor names: {', '.join(self.factor_cols)}")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Phase 1 Data Analysis Tool")
    logger.info("=" * 80)

    # Step 1: Input data file path
    data_csv = input(
        "Enter experimental data CSV path (or press Enter for default 'warmup_data.csv'): "
    ).strip()
    if not data_csv:
    ...
    if not Path(data_csv).exists():
        ...
        logger.info("Hint: Ensure CSV contains the following columns:")
        logger.info("  - Subject ID column (default: subject_id)")
        logger.info("  - Response variable column (default: response)")
        logger.info("  - All factor columns")
        sys.exit(1)

    # Step 2: Input column names
    logger.info("Specify column names:")
    subject_col = input("  Subject ID column name (default 'subject_id'): ").strip() or "subject_id"
    response_col = input("  Response variable column name (default 'response'): ").strip() or "response"
    ...
    logger.info("Configure analysis parameters:")
    try:
        max_pairs = int(input("  Max interaction pairs (default 5): ").strip() or "5")
        min_pairs = int(input("  Min interaction pairs (default 3): ").strip() or "3")
    except ValueError:
        logger.error("Input must be an integer")
        sys.exit(1)

    selection_method = (
        input("  Selection method (elbow/bic_threshold/top_k, default elbow): ").strip() or "elbow"
    )
    ...
    # Step 6: Configure Phase 2
    logger.info("=" * 80)
    logger.info("Phase 2 Configuration")
    logger.info("=" * 80)
    logger.info("Enter Phase 2 parameters:")
    try:
        n_subjects = int(input("  Number of subjects: "))
        trials_per_subject = int(input("  Trials per subject: "))
        lambda_adjustment_str = input("  λ adjustment factor (default 1.2): ").strip()
        lambda_adjustment = (
    ...
    # Step 7: Generate Phase 2 config
    try:
        ...
        logger.info("=" * 80)
        logger.info("Phase 2 Configuration Generated")
        logger.info("=" * 80)
        logger.info(f"Total budget: {phase2_config['total_budget']} trials")
        logger.info(f"Selected interaction pairs: {len(phase2_config['interaction_pairs'])}")
        logger.info(
            f"λ initial: {phase2_config['lambda_init']:.3f} (Phase 1: {phase2_config['lambda_phase1']:.3f})"
        )
        logger.info(f"λ end: {phase2_config['lambda_end']:.3f}")
        logger.info(f"γ initial: {phase2_config['gamma_init']:.3f}")
        logger.info(f"γ end: {phase2_config['gamma_end']:.3f}")
        logger.info(f"Mid-diagnostic: Trial {phase2_config['mid_diagnostic_trial']}")

    except Exception as e:
        logger.error(f"Failed to generate Phase 2 config: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
    ...
    output_dir = (
        input("Output directory (default 'phase1_analysis_output'): ").strip()
        or "phase1_analysis_output"
    )
    prefix = input("Filename prefix (default 'phase1'): ").strip() or "phase1"
    ...
    try:
        ...
        logger.info("1. View analysis report: " + exported_files["report"])
        logger.info("2. Read usage guide: " + exported_files["usage_guide"])
        logger.info("3. Load config in Phase 2:")
        logger.info(f"   - JSON: {exported_files['json_config']}")
        logger.info(f"   - NumPy: {exported_files['npz_schedules']}")
