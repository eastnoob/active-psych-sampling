...
STEP1_CONFIG = {
    ...
    "output_dir": r"extensions\warmup_budget_check\sample\202511161637",
    "merge": False,
}
...
def import_subject_module():
    ...
    # Last attempt: add repository root guessed by walking 6 levels up
...
def convert_row_to_features(row):
    ...
    The conversion mirrors typical normalization but keeps discrete levels numeric.
    """
    ...
    # x2 is 1-5 discrete: convert to float
...
def run(
    ...
    with report_path.open("w", encoding="utf-8") as rp:
        ...
        rp.write(f"- Output mode: {final_output_mode}\n")
        rp.write(f"- Use latent variables: {final_use_latent}\n")
        rp.write(f"- Output type: {final_output_type}\n")
        if final_output_type == "likert":
            rp.write(f"- Likert levels: {final_likert_levels}\n")
            rp.write(f"- Likert mapping mode: {final_likert_mode}\n")
            rp.write(f"- Likert sensitivity: {final_likert_sensitivity}\n")
        ...
        rp.write(f"- Clean prior results: {final_clean}\n\n")
        ...
        if final_output_type == "likert":
            rp.write(f"- Output range: Likert 1-{final_likert_levels}\n")
        else:
        ...
        if final_output_type == "likert" and Path(dist_path).exists():
            rp.write(
