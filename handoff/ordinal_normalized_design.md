...
**问题**: ANOVA效应分解能否正确使用ordinal参数的间距信息？
...
1. **隐含支持机制**：
   - `LocalSampler.sample()` 返回 **值空间点**，而非rank编码
   - GP模型在值空间观测这些点（e.g., 2.0m, 2.5m, 3.5m）
   - GP核函数（RBF等）自动学到空间结构：2.0与2.5距离0.5 < 2.5与3.5距离1.0
   - ANOVA效应分解：Δ = I(x_perturbed) - I(x)，差异自动反映间距

2. **ANOVA的角色**：
   - ANOVA 不显式计算间距，只分解模型后验
   - 但间距关系已通过采样点编码进GP后验中
   - 当计算参数效应时，不同间距导致不同的后验变化

3. **具体例子**：

   ```
   参数：天花板高度 [2.0m, 2.5m, 3.5m]
   
   scenario 1: 扰动到2.5m
     → GP看到增量 Δ_height = Var(2.5m) - Var(2.0m) 
   
   scenario 2: 扰动到3.5m  
     → GP看到增量 Δ_height = Var(3.5m) - Var(2.0m)
   
   间距关系保留：
     因为2.5与3.5的采样密度差异会影响GP的学习轨迹
     ANOVA自动在值空间中工作，无需显式处理
   ```

### 关键实现约束

**必须**：`LocalSampler._perturb_ordinal()` 返回 **原始值**，而非rank

```python
# ❌ 错误
def _perturb_ordinal(self, base, k, B):
    # ... 扰动逻辑 ...
    base[:, :, k] = rank_values  # 返回 [0, 1, 2]
    # 🔴 间距信息丢失！GP看不到0.5m vs 1.0m的差异

# ✅ 正确
def _perturb_ordinal(self, base, k, B):
    # 1. base[k] 包含原始值（e.g., 2.5）
    # 2. 在rank空间内扰动+舍入
    # 3. 最近邻约束映射 rank' → 原始值
    base[:, :, k] = original_values  # 返回 [2.0, 2.5, 3.5] 之一
    # 🟢 间距信息保留！GP学到完整的0.5m, 1.0m结构
```

**结论**：当前EUR-ANOVA实现原生支持ordinal的间距保留，无需修改ANOVA代码。
...
- Pool parameter is optional in LocalSampler.**init**()
