...
**关键约束**: `LocalSampler._perturb_ordinal()` 必须返回 **原始值**，而非rank编码

```python
# LocalSampler._perturb_ordinal() 的返回值要求
def _perturb_ordinal(self, base, k, B):
    # base[:,:,k] 包含原始值（e.g., 2.5 for 天花板高度）
    # 
    # ✅ 正确流程：
    # 1. 获取ordinal值集: values = [2.0, 2.5, 3.5]
    # 2. 映射到rank: 2.5 → rank=1
    # 3. 扰动rank: 1 + N(0,σ) → 1.2
    # 4. 舍入+约束: round(1.2) = 1
    # 5. 映射回原始值: rank=1 → 2.5 ✅
    # 6. base[:,:,k] = 2.5  # 返回原始值！
    # 
    # ❌ 错误：
    # base[:,:,k] = 1  # 返回rank - 会丢失间距信息！
    
    # 这样间距信息才能被保留给GP和ANOVA使用
    pass
```
...
- GP和ANOVA在值空间中工作
- 只有保留原始值，间距信息（0.5m vs 1.0m）才能被编码进后验
- 间距信息对ANOVA效应分解正确性至关重要
