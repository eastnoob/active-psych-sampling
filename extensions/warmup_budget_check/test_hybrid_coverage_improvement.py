...
def compute_single_factor_coverage(design_df: pd.DataFrame, selected_indices: List[int]) -> dict:
    """
    计算单因子水平覆盖度
...
def compute_pairwise_coverage(design_df: pd.DataFrame, selected_indices: List[int]) -> dict:
    """
    计算因子对的值组合覆盖度
...
def simulate_random_sampling(design_df: pd.DataFrame, n_free: int, protected_configs: List[int],
                            used_indices: Set[int], n_trials: int = 100) -> dict:
    """
    模拟纯随机采样（改进前的方法）

    Args:
        n_trials: 重复试验次数，用于评估随机性带来的波动
    ...
    for _ in range(n_trials):
        ...
        # 计算覆盖度
        single_cov = compute_single_factor_coverage(design_df, free_exploration)
        pairwise_cov = compute_pairwise_coverage(design_df, free_exploration)
...
def run_comparison_test():
    ...
    print(f"  因子对数量: {n_factors * (n_factors - 1) // 2}")
    print()
    ...
    # 计算保护配置对指定交互对的覆盖度
    protected_pairwise_cov = compute_pairwise_coverage(design_df, protected_configs)
    for pair in interaction_pairs:
        cov = protected_pairwise_cov.get(tuple(sorted(pair)), 0.0)
        print(f"  指定对 {pair} 覆盖度: {cov*100:.1f}%")
    print()
    ...
    random_results = simulate_random_sampling(
        design_df, n_free, protected_configs, used_indices, n_trials=100
    )
    ...
    # 计算覆盖度
    improved_single_cov = compute_single_factor_coverage(design_df, free_exploration_improved)
    improved_pairwise_cov = compute_pairwise_coverage(design_df, free_exploration_improved)
    ...
    random_pairwise_cov = compute_pairwise_coverage(design_df, free_exploration_random)

    # 找出未保护的交互对
    protected_pairs_set = set(tuple(sorted(pair)) for pair in interaction_pairs)

    print(f"{'交互对':<15} {'改进前 (随机)':<20} {'改进后 (优化)':<20} {'提升':<15}")
    print("-" * 70)
    ...
    for i in range(n_factors):
        for j in range(i + 1, n_factors):
            ...
            if pair not in protected_pairs_set:
                ...
                print(f"{str(pair):<15} {random_cov*100:>6.2f}%            {improved_cov*100:>6.2f}%            {improvement:>+6.2f}%")
