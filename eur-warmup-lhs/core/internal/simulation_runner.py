...
def run(
    ...
    logger.debug(f"run() called with:")
    logger.debug(f"  - interaction_pairs: {interaction_pairs}")
    logger.debug(f"  - num_interactions: {num_interactions}")
    logger.debug(f"  - interaction_scale: {interaction_scale}")
    logger.debug(f"args object attributes:")
    logger.debug(
        f"  - args.interaction_pairs: {getattr(args, 'interaction_pairs', 'NOT SET')}"
    )
    logger.debug(f"  - args.num_interactions: {getattr(args, 'num_interactions', 'NOT SET')}")
    logger.debug(
    ...
    if not csvs:
        logger.error(f"No subject CSV files found in {input_dir}")
        return
    ...
    print("Each subject will independently generate population_weights and individual_deviations.")
    print(f"Population distribution: mean={args.population_mean}, std={args.population_std}")
    print(f"Individual std: {args.individual_std_percent * args.population_std:.4f} (percent={args.individual_std_percent})")
    ...
    if args.fixed_weights_file:
        ...
        if not fw_path.exists():
            logger.error(f"Fixed weights file not found: {fw_path}")
            fw_path = None
        else:
            if fw_path.suffix.lower() in {".json"}:
                ...
                logger.error("Unsupported fixed_weights file type, use .json or .npy")
                fixed_weights_map = None
    else:
        ...
        logger.info(f"Wrote default fixed_weights to {default_path}")
    ...
    for idx, csv_path in enumerate(csvs, start=1):
        ...
        if idx == 1:
            logger.debug(f"Subject {idx} - interaction_pairs: {getattr(args, 'interaction_pairs', None)}")
        ...
        logger.info(f"Wrote model description for {csv_path.name} -> {spec_md_path.name}")
    ...
    if output_mode in ("individual", "both"):
        for subj_name, rows in subject_outputs.items():
            ...
            logger.info(f"Wrote result for {subj_name} -> {out_path.name}")
    ...
    logger.info(f"Saved model spec to {spec_path}")
    ...
    print(f"Wrote subjects parameters summary to {summary_md}")
    ...
    if args.output_type == "likert":
        ...
        print(f"Wrote Likert distribution summary to {dist_path}")
    ...
    with report_path.open("w", encoding="utf-8") as rp:
        ...
        # (I'll just replace the final print)
    # (Wait, I need to match the exact block)
    ...
    print(f"Wrote comprehensive simulation report to {report_path}")
